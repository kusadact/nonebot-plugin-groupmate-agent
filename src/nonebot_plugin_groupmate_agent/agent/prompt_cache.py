...
def should_use_explicit_prompt_cache(config: Any) -> bool:
    if not getattr(config, "chat_explicit_prompt_cache", False):
        return False

    api_format = str(getattr(config, "chat_api_format", "") or "").strip().lower()
    if api_format == "anthropic":
        return True

    base_url = str(getattr(config, "chat_base_url", "") or "").strip().lower()
...
def build_system_messages(
    ...
    *,
    ...
    stable_content = _cacheable_text_block(stable_prompt) if use_cache_control else stable_prompt
