...
def _render_dashboard(data: dict, *, path: str, token: str | None, config: ScopedConfig) -> str:
    ...
    session_rows = [
        [
            f"<code>{escape(_text(row['session_id']))}</code>",
            escape(_text(row["session_type"])),
    ...
    user_rows = [
        [
            f"<code>{escape(_text(row['user_id']))}</code>",
            escape(_text(row["user_name"])),
            _fmt_int(row["requests"]),
            _fmt_int(row["total_tokens"]),
            _fmt_int(row["cached_tokens"]),
            _fmt_int(row["cache_creation_tokens"]),
    ...
    recent_rows = [
        [
            escape(_text(row["created_at"])[:19].replace("T", " ")),
            f"<code>{escape(_text(row['session_id']))}</code>",
            f"<code>{escape(_text(row['user_id']))}</code>",
            escape(_text(row["user_name"])),
            escape(_text(row["model"])),
...
<head>
  ...
  <title>Groupmate Agent Usage</title>
  <style>
    :root {{
      color-scheme: light;
      --bg: #f7f8fa;
    ...
    * {{ box-sizing: border-box; }}
    ...
    .metrics {{ display: grid; grid-template-columns: repeat(7, minmax(120px, 1fr)); gap: 12px; margin: 18px 0 22px; }}
...
<body>
  <header>
    <h1>Groupmate Agent Token Usage</h1>
  ...
  <main>
    ...
    <div class="grid">
      <section><h2>按群/会话</h2><div class="table-wrap">{_render_table(["会话 ID", "类型", "请求", "Tokens", "命中", "创建", "费用"], session_rows)}</div></section>
      <section><h2>按用户</h2><div class="table-wrap">{_render_table(["用户 ID", "名称", "请求", "Tokens", "命中", "创建", "费用"], user_rows)}</div></section>
    </div>
    <section><h2>按模型</h2><div class="table-wrap">{_render_table(["模型", "请求", "输入", "输出", "命中", "创建", "总计", "费用"], model_rows)}</div></section>
    <section><h2>最近请求</h2><div class="table-wrap">{_render_table(["时间", "会话", "用户", "名称", "模型", "Tokens", "命中", "创建", "费用"], recent_rows)}</div></section>
  </main>
</body>
</html>"""
...
def register_usage_webui(config: ScopedConfig) -> None:
    ...
    if not config.usage_webui_token:
        logger.warning("Token 用量 WebUI 已开启但未设置 usage_webui_token，请确认访问路径没有暴露到公网")
